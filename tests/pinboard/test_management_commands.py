...
class FetchPinboardArgs(TestCase):
    ...
    def test_with_date(self, fetch_method):
        ...
        call_command("fetch_pinboard_bookmarks", date="2015-06-20", stdout=StringIO())
        fetch_method.assert_called_once_with(post_date="2015-06-20", username=None)
    ...
    def test_with_url_and_account(self, fetch_method):
        ...
        url = "http://new-aesthetic.tumblr.com/"
        call_command(
            "fetch_pinboard_bookmarks", url=url, account="philgyford", stdout=StringIO()
        )
...
class FetchPinboardOutput(TestCase):
    ...
    def test_error_output(self, fetch_method):
        ...
        out = StringIO()
        out_err = StringIO()
        call_command("fetch_pinboard_bookmarks", all=True, stdout=out, stderr=out_err)
        self.assertIn(
            "philgyford: Failed to fetch Bookmarks: It broke", out_err.getvalue()
        )
    ...
    def test_multiple_account_output(self, fetch_method):
        ...
        self.assertIn("philgyford: Fetched 23 Bookmarks", out.getvalue())
        self.assertIn(
            "wrongaccount: Failed to fetch Bookmarks: It broke", out_err.getvalue()
        )
