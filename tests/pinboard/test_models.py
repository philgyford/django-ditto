...
class AccountTestCase(TestCase):
    ...
    def test_unique_url(self):
        ...
        AccountFactory(url="https://pinboard.in/u:billy")
        with self.assertRaises(IntegrityError):
            AccountFactory(url="https://pinboard.in/u:billy")
    ...
    def test_ordering(self):
        ...
        AccountFactory(username="billy")
        AccountFactory(username="amanda")
        accounts = Account.objects.all()
        self.assertEqual(accounts[0].username, "amanda")

    def test_public_bookmarks_count(self):
        account = AccountFactory()
        BookmarkFactory.create_batch(3, account=account, is_private=False)
        BookmarkFactory.create_batch(2, account=account, is_private=True)
...
class BookmarkTestCase(TestCase):
    ...
    def test_get_absolute_url(self):
        bookmark = BookmarkFactory(
            account=AccountFactory(username="bob"), url_hash="12345678abc"
        )
        self.assertEqual(bookmark.get_absolute_url(), "/pinboard/bob/12345678abc/")

    def test_get_absolute_url_2(self):
        "Has the correct URL on this site"
        account = AccountFactory(username="billy")
        bookmark = BookmarkFactory(account=account, url="http://www.example.com")
        self.assertEqual(bookmark.get_absolute_url(), "/pinboard/billy/847310eb455f/")
    ...
    def test_url_constraint(self):
        ...
        account = AccountFactory()
        bookmark_1 = BookmarkFactory(account=account, url="http://www.example.com")
        bookmark_1.save()
        with self.assertRaises(IntegrityError):
            BookmarkFactory(account=account, url="http://www.example.com")

    def test_url_unconstrained(self):
        """URLs do not have to be unique for different Accounts' Bookmarks"""
        account_1 = AccountFactory()
        bookmark_1 = BookmarkFactory(account=account_1, url="http://www.example.com")
        bookmark_1.save()
    ...
    def test_default_manager(self):
        ...
        BookmarkFactory(is_private=False)
        BookmarkFactory(is_private=True)
        BookmarkFactory(is_private=False)
    ...
    def test_post_year(self):
        ...
        bookmark = BookmarkFactory(post_time=datetime_from_str("2015-01-01 12:00:00"))
...
class BookmarkNextPrevTestCase(TestCase):
    ...
    def test_next_public_by_post_time(self):
        self.assertEqual(
            self.bookmark_1.get_next_public_by_post_time(), self.bookmark_2
        )
    ...
    def test_previous_public_by_post_time(self):
        self.assertEqual(
            self.bookmark_2.get_previous_public_by_post_time(), self.bookmark_1
        )
