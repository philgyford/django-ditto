...
class PinboardViewTests(TestCase):
    ...
    def test_home_context(self):
        ...
        AccountFactory(username="charlie")
        BookmarkFactory.create_batch(5, account=account_1)
        BookmarkFactory.create_batch(5, account=account_2)
        response = self.client.get(reverse("pinboard:home"))
    ...
    def test_account_detail_context(self):
        ...
        BookmarkFactory.create_batch(3, account=account_1)
        BookmarkFactory.create_batch(3, account=account_2)
        response = self.client.get(
            reverse("pinboard:account_detail", kwargs={"username": account_1.username})
        )
        ...
        self.assertEqual(
            [bookmark.pk for bookmark in response.context["bookmark_list"]], [3, 2, 1]
        )
    ...
    def test_account_detail_privacy(self):
        ...
        BookmarkFactory(account=account, is_private=True)
        response = self.client.get(
            reverse("pinboard:account_detail", kwargs={"username": account.username})
        )
        self.assertEqual(len(response.context["bookmark_list"]), 1)
        self.assertTrue(response.context["bookmark_list"][0].pk, public_bookmark.pk)
    ...
    def test_account_toread_context(self):
        ...
        response = self.client.get(
            reverse(
                "pinboard:account_toread", kwargs={"username": accounts[0].username}
            )
        )
        self.assertIn("account", response.context)
        ...
        self.assertIn("bookmark_list", response.context)
        self.assertEqual(len(response.context["bookmark_list"]), 1)
        self.assertEqual(response.context["bookmark_list"][0].pk, bookmarks_1[0].pk)
    ...
    def test_account_tag_detail_context(self):
        ...
        self.assertEqual(len(response.context["bookmark_list"]), 2)
        self.assertEqual(
            [bookmark.pk for bookmark in response.context["bookmark_list"]], [2, 1]
        )
