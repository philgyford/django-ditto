...
class FetchTypesTestRemoteCase(FetchTestCase):
    ...
    def make_success_body(
        self, method="date", num_posts=1, post_date="2015-06-18", username="philgyford"
    ):
    ...
    def test_fetch_all_success(self):
        ...
        self.add_response(
            method="all", body=self.make_success_body(method="all", num_posts=12)
        )
    ...
    def test_fetch_recent_success(self):
        ...
        self.add_response(method="recent", body=self.make_success_body(num_posts=5))
    ...
    def test_fetch_date_invalid(self, fetch_method):
        ...
        with self.assertRaises(FetchError):
            DateBookmarksFetcher().fetch("2015-06-99")
    ...
    def test_fetch_multiple_accounts(self):
        ...
        self.add_response(
            body=self.make_success_body(num_posts=3, username="philgyford")
        )
        self.add_response(body=self.make_success_body(num_posts=3, username="testuser"))
...
class FetchInactiveAccountsTestCase(TestCase):
    def test_only_inactive_account(self):
        ...
        AccountFactory(username="philgyford", is_active=False)
        with self.assertRaises(FetchError):
            RecentBookmarksFetcher().fetch(username="philgyford")

    def test_inactive_accounts(self):
        "Correctly reacts fetching all accounts but all are inactive"
        AccountFactory(username="philgyford", is_active=False)
        AccountFactory(username="testuser", is_active=False)
        with self.assertRaises(FetchError):
            RecentBookmarksFetcher().fetch()
...
class FetchTypesSaveTestCase(FetchTestCase):
    ...
    def test_fetch_json_parsing(self):
        ...
        self.assertEqual(
            bookmarks_data[0]["tags"], ["fonts", "icons", "webdevelopment", "webdesign"]
        )
    ...
    def test_save_bookmarks(self):
        ...
        BookmarksFetcher()._save_bookmarks(
            account=account, bookmarks_data=bookmarks_data, fetch_time=fetch_time
        )
