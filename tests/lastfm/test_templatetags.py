...
class TopAlbumsTestCase(TestCase):
    def setUp(self):
        ...
        for idx, track in enumerate(self.tracks):
            ScrobbleFactory(artist=self.artist1, track=track, album=self.albums[idx])

        # Extra scrobbles.
        # For artist1, tracks[1] will be 1st, tracks[2] will be 2nd:
        ScrobbleFactory.create_batch(
            2, artist=self.artist1, track=self.tracks[1], album=self.albums[1]
        )
        ScrobbleFactory.create_batch(
            1, artist=self.artist1, track=self.tracks[2], album=self.albums[2]
        )
        ...
        ScrobbleFactory.create_batch(
            4, artist=self.artist2, track=self.track2, album=self.album2
        )
    ...
    def test_for_day_with_datetime(self):
        ...
        albums = ditto_lastfm.top_albums(
            period="day", date=datetime_from_str("2016-10-01 00:00:00")
        )
    ...
    def test_for_day_with_date(self):
        ...
        albums = ditto_lastfm.top_albums(
            period="day", date=datetime_from_str("2016-10-01 00:00:00").date()
        )
        self.assertEqual(len(albums), 1)
        self.assertEqual(albums[0].scrobble_count, 2)
    ...
    def test_for_week_sunday_start(self):
        ...
        albums = ditto_lastfm.top_albums(
            period="week", date=datetime_from_str("2017-09-07 00:00:00")
        )
        self.assertEqual(len(albums), 1)
        self.assertEqual(albums[0].scrobble_count, 2)
    ...
    def test_for_week_monday_start(self):
        ...
        ScrobbleFactory(
            track=self.tracks[1],
            album=self.albums[1],
            post_time=datetime_from_str("2017-09-04 12:00:00"),
        )
        ScrobbleFactory(
            track=self.tracks[1],
            album=self.albums[1],
    ...
    def test_for_month(self):
        ...
        albums = ditto_lastfm.top_albums(
            period="month", date=datetime_from_str("2016-09-15 00:00:00")
        )
        self.assertEqual(len(albums), 1)
        self.assertEqual(albums[0].scrobble_count, 2)
    ...
    def test_for_year(self):
        ...
        albums = ditto_lastfm.top_albums(
            period="year", date=datetime_from_str("2015-09-01 00:00:00")
        )
        self.assertEqual(len(albums), 1)
        self.assertEqual(albums[0].scrobble_count, 2)
...
class TopArtistsTestCase(TestCase):
    def setUp(self):
        ...
        ScrobbleFactory.create_batch(2, artist=self.artists[1], track=self.tracks[1])
        ScrobbleFactory.create_batch(1, artist=self.artists[2], track=self.tracks[2])
    ...
    def test_for_account(self):
        ...
        ScrobbleFactory(account=account, artist=self.artists[1], track=self.tracks[1])
...
class TopTracksTestCase(TestCase):
    def setUp(self):
        ...
        # For artist1, tracks[1] will be 1st, tracks[2] will be 2nd:
        ScrobbleFactory.create_batch(
            2, album=self.album1, artist=self.artist1, track=self.tracks[1]
        )
        ScrobbleFactory.create_batch(
            1, album=self.album1, artist=self.artist1, track=self.tracks[2]
        )
        ...
        self.track2 = TrackFactory(artist=self.artist2)
        ScrobbleFactory.create_batch(
            4, album=self.album2, artist=self.artist2, track=self.track2
        )
    ...
    def test_for_account(self):
        ...
        account = AccountFactory()
        # Our one scrobble by this account:
        ScrobbleFactory(account=account, artist=self.artist1, track=self.tracks[1])
    ...
    def test_for_day_with_datetime(self):
        ...
        # 2 scrobbles on this day:
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2016-10-01 00:00:00")
        )
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2016-10-01 23:59:59")
        )
        # 1 scrobble on this day:
        ScrobbleFactory(
            track=self.tracks[2], post_time=datetime_from_str("2016-10-02 12:00:00")
        )
    ...
    def test_for_week_sunday_start(self):
        ...
        # 2017-09-03 is a Sunday
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2017-09-03 00:00:00")
        )
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2017-09-09 23:59:59")
        )
        # 1 scrobble on this day:
        ScrobbleFactory(
            track=self.tracks[2], post_time=datetime_from_str("2017-09-10 12:00:00")
        )
    ...
    def test_for_week_monday_start(self):
        ...
        # 2017-09-04 is a Monday
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2017-09-04 00:00:00")
        )
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2017-09-10 23:59:59")
        )
        # 1 scrobble on this day:
        ScrobbleFactory(
            track=self.tracks[2], post_time=datetime_from_str("2017-09-11 12:00:00")
        )
    ...
    def test_for_month(self):
        ...
        # 2 scrobbles in this month:
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2016-09-01 12:00:00")
        )
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2016-09-30 12:00:00")
        )
        # 1 scrobble in this month:
        ScrobbleFactory(
            track=self.tracks[2], post_time=datetime_from_str("2016-10-01 12:00:00")
        )
    ...
    def test_for_year(self):
        ...
        # 2 scrobbles in this year:
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2015-01-01 12:00:00")
        )
        ScrobbleFactory(
            track=self.tracks[1], post_time=datetime_from_str("2015-12-31 12:00:00")
        )
        # 1 scrobble in this year:
        ScrobbleFactory(
            track=self.tracks[2], post_time=datetime_from_str("2016-03-01 12:00:00")
        )
...
class RecentScrobblesTestCase(TestCase):
    def setUp(self):
        ...
        ScrobbleFactory.create_batch(11, account=self.account1)
        self.scrobble2 = ScrobbleFactory(account=self.account2)

    def test_for_account(self):
        "With an Account, it gets its recent scrobbles"
        scrobbles = ditto_lastfm.recent_scrobbles(account=self.account1, limit=12)
...
class AnnualScrobbleCountsTestCase(TestCase):
    def setUp(self):
        ...
        ScrobbleFactory.create_batch(
            3, post_time=datetime_from_str("2015-01-01 12:00:00"), account=self.account1
        )
        ScrobbleFactory.create_batch(
            2, post_time=datetime_from_str("2016-01-01 12:00:00"), account=self.account1
        )
        # And one for account2 in 2015:
        ScrobbleFactory(
            account=self.account2, post_time=datetime_from_str("2015-01-01 12:00:00")
        )
...
class DayScrobblesTestCase(TestCase):
    def setUp(self):
        ...
        self.scrobble1 = ScrobbleFactory(
            post_time=datetime_from_str("2016-10-01 12:00:00"), account=self.account1
        )
        self.scrobble2 = ScrobbleFactory(
            post_time=datetime_from_str("2016-10-01 12:05:00"), account=self.account1
        )
        # One for account1 on 2nd Oct:
        self.scrobble3 = ScrobbleFactory(
            post_time=datetime_from_str("2016-10-02 00:00:01"), account=self.account1
        )
        # And one for account2 on 1st Oct:
        self.scrobble4 = ScrobbleFactory(
            account=self.account2, post_time=datetime_from_str("2016-10-01 13:00:00")
        )

    def test_for_all_accounts(self):
        "Returns ALL scrobbles from requested date."
        scrobbles = ditto_lastfm.day_scrobbles(datetime_from_str("2016-10-01 00:00:00"))
        self.assertEqual(len(scrobbles), 3)
    ...
    def test_for_one_accounts(self):
        ...
        scrobbles = ditto_lastfm.day_scrobbles(
            account=self.account1, date=datetime_from_str("2016-10-01 00:00:00")
        )
        self.assertEqual(len(scrobbles), 2)
        self.assertEqual(scrobbles[0], self.scrobble1)
        self.assertEqual(scrobbles[1], self.scrobble2)
    ...
    def test_date_error_no_date(self):
        "Should raise TypeError if no date is supplied."
        with self.assertRaises(TypeError):
            ditto_lastfm.day_scrobbles()

    def test_date_error_invalid_date(self):
        "Should raise TypeError if invalid date is supplied."
        with self.assertRaises(TypeError):
