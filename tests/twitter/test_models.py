...
class AccountTestCase(TestCase):
    ...
    def test_creates_user(self, fetch_avatar):
        ...
        self.add_response(
            body=self.make_verify_credentials_body(), call="account/verify_credentials"
        )
...
class PhotoTestCase(TestCase):
    ...
    def test_default_manager(self):
        ...
        PhotoFactory.create_batch(2, tweets=(public_tweet,))
        PhotoFactory.create_batch(1, tweets=(private_tweet,))
...
class PhotoLocalTestCase(TestCase):
    ...
    def test_broken_image(self):
        ...
        with patch.dict("ditto.twitter.models.Media.IMAGE_SIZES", image_sizes):
            self.assertEqual(photo.small_url, "/static/img/original_error.jpg")
...
class VideoTestCase(TestCase):
    ...
    def test_video_url_dash(self):
        video = VideoFactory(xmpeg_url="", dash_url="https://example.org/test.mpd")
        self.assertEqual(video.video_url, video.dash_url)

    def test_video_url_mp4(self):
        video = VideoFactory(
            xmpeg_url="", dash_url="", mp4_url="https://example.org/test.mp4"
        )
...
class TweetTestCase(TestCase):
    ...
    def test_get_absolute_url(self):
        tweet = TweetFactory(user=UserFactory(screen_name="bob"), twitter_id=1234567890)
    ...
    def test_default_manager_recent(self):
        ...
        public_user = UserFactory(is_private=False)
        private_user = UserFactory(is_private=True)
        TweetFactory.create_batch(2, user=public_user)
        TweetFactory(user=private_user)
    ...
    def test_in_reply_to_url(self):
        tweet_1 = TweetFactory(
            in_reply_to_screen_name="bob", in_reply_to_status_id=1234567
        )
        self.assertEqual(
            tweet_1.in_reply_to_url, "https://twitter.com/bob/status/1234567"
        )
        tweet_2 = TweetFactory(in_reply_to_screen_name="")
    ...
    def test_get_quoted_tweet(self):
        TweetFactory(text="The quote!", twitter_id=123)
    ...
    def test_get_retweeted_tweet(self):
        TweetFactory(text="Retweeted tweet!", twitter_id=123)
...
class TweetNextPrevTestCase(TestCase):
    ...
    def test_previous_public_by_post_time(self):
        self.assertEqual(self.tweet_2.get_previous_public_by_post_time(), self.tweet_1)
...
class UserTestCase(TestCase):
    ...
    def test_with_accounts_manager(self):
        ...
        AccountFactory(user=users[0])
        AccountFactory(user=users[2])
    ...
    def test_makes_description_html(self, htmlify_method):
        ...
        user.save()
        htmlify_method.assert_called_once_with({"description": "my test description"})
