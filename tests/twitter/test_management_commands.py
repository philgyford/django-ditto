...
class FetchTwitterFavoritesArgs(FetchTwitterArgs):
    ...
    def test_with_favorites_new(self):
        "Calls the correct method when fetching favorite new tweets"
    ...
    def test_with_favorites_int(self):
        "Should send an int to fetch()"
...
class FetchTwitterTweetsOutput(FetchTwitterOutput):
    ...
    def test_success_output_verbosity_0(self):
        ...
        call_command("fetch_twitter_tweets", recent="new", verbosity=0, stdout=self.out)
        self.assertEqual("", self.out.getvalue())

    def test_error_output(self):
        "Responds correctly when there was an error fetching recent tweets"
        self.fetch_method.side_effect = [
            [{"account": "philgyford", "success": False, "messages": ["It broke"]}]
        ]
        call_command(
            "fetch_twitter_tweets", recent="new", stdout=self.out, stderr=self.out_err
        )
        self.assertIn(
            "philgyford: Failed to fetch Tweets: It broke", self.out_err.getvalue()
        )
...
class FetchTwitterFavoritesOutput(FetchTwitterOutput):
    ...
    def test_success_output_verbosity_0(self):
        ...
        call_command(
            "fetch_twitter_favorites", recent="new", verbosity=0, stdout=self.out
        )
        self.assertEqual("", self.out.getvalue())
...
class FetchTwitterAccountsOutput(FetchTwitterOutput):
    ...
    def test_success_output(self):
        ...
        self.fetch_method.side_effect = [[{"account": "philgyford", "success": True}]]
...
class GenerateTweetHtml(TestCase):
    def setUp(self):
        ...
        factories.TweetFactory.create_batch(2, user=user_1)
        factories.TweetFactory.create_batch(3, user=user_2)
        factories.AccountFactory(user=user_1)
        factories.AccountFactory(user=user_2)
        self.out = StringIO()
    ...
    def test_with_one_account(self, save_method):
        call_command("generate_twitter_tweet_html", account="terry", stdout=self.out)
...
class UpdateUsers(TestCase):
    def setUp(self):
        ...
        user_2 = factories.UserFactory(screen_name="bob")
        factories.AccountFactory(user=user_1)
        factories.AccountWithCredentialsFactory(user=user_2)

        self.patcher = patch(
            "ditto.twitter.management.commands.update_twitter_users.UsersFetcher"
        )
        self.fetcher_class = self.patcher.start()
        self.out = StringIO()
        self.out_err = StringIO()
    ...
    def test_success_output_verbosity_0(self):
        ...
        call_command(
            "update_twitter_users", account="bob", verbosity=0, stdout=self.out
        )
        self.assertEqual("", self.out.getvalue())
    ...
    def test_error_output(self):
        ...
        call_command(
            "update_twitter_users", account="bob", stdout=self.out, stderr=self.out_err
        )
        self.assertIn(
            "philgyford: Failed to fetch Users: It broke", self.out_err.getvalue()
        )
...
class UpdateTweets(TestCase):
    ...
    def test_success_output_verbosity_0(self):
        ...
        call_command(
            "update_twitter_tweets", account="bob", verbosity=0, stdout=self.out
        )
        self.assertEqual("", self.out.getvalue())
    ...
    def test_error_output(self):
        ...
        call_command(
            "update_twitter_tweets", account="bob", stdout=self.out, stderr=self.out_err
        )
        self.assertIn(
            "philgyford: Failed to fetch Tweets: It broke", self.out_err.getvalue()
        )
...
class FetchFiles(TestCase):
    ...
    def test_success_output(self, fetcher):
        fetcher.return_value.fetch.return_value = [{"success": True, "fetched": 33}]
    ...
    def test_error_output(self, fetcher):
        ...
        call_command("fetch_twitter_files", stdout=self.out, stderr=self.out_err)
