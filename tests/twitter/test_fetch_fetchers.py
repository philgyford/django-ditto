...
class TwitterFetcherTestCase(FetchTwitterTestCase):
    ...
    def test_raises_error_with_invalid_screen_name(self):
        ...
        with self.assertRaises(FetchError):
            RecentTweetsFetcher(screen_name="badname")
...
class RecentTweetsFetcherTestCase(TwitterFetcherTestCase):
    ...
    def test_api_request_for_one_account(self, download):
        ...
        RecentTweetsFetcher(screen_name="jill").fetch()
    ...
    def test_includes_count(self):
        ...
        with patch.object(FetchTweetsRecent, "_save_results"):
            RecentTweetsFetcher(screen_name="jill").fetch(count=25)
    ...
    def test_returns_error_if_api_call_fails(self):
        self.add_response(
            body='{"errors":[{"message":"Rate limit exceeded","code":88}]}', status=429
        )
    ...
    def test_returns_error_if_no_creds(self):
        ...
        self.assertFalse(result[0]["success"])
        self.assertIn("Account has no API credentials", result[0]["messages"][0])
    ...
    def test_fetches_multiple_pages_for_new(self, download):
        ...
        self.add_response(
            body=self.make_response_body(), querystring=qs, match_querystring=True
        )
        qs["max_id"] = 99
...
class FavoriteTweetsFetcherTestCase(TwitterFetcherTestCase):
    ...
    def test_api_request_for_one_account(self, download):
        ...
        self.add_response(body=self.make_response_body())
        FavoriteTweetsFetcher(screen_name="jill").fetch()
        self.assertEqual(1, len(responses.calls))
    ...
    def test_includes_count(self):
        ...
        with patch.object(FetchTweetsFavorite, "_save_results"):
            FavoriteTweetsFetcher(screen_name="jill").fetch(count=25)
            self.assertIn("count=25", responses.calls[0][0].url)
            self.assertNotIn("since_id=100", responses.calls[0][0].url)
...
class UsersFetcherTestCase(TwitterFetcherTestCase):
    ...
    def test_makes_one_api_call(self, fetch_avatar):
        self.add_response(body=self.make_response_body())
        UsersFetcher(screen_name="jill").fetch([460060168, 26727655, 6795192])
        self.assertEqual(1, len(responses.calls))
    ...
    def test_makes_correct_api_call(self):
        ...
        self.assertIn(
            "user_id=460060168%2C26727655%2C6795192", responses.calls[0][0].url
        )
    ...
    def test_requests_all_users(self):
        ...
        UsersFetcher(screen_name="jill").fetch()
    ...
    def test_updates_users(self):
        UserFactory.create(twitter_id=26727655, screen_name="bill")
        self.add_response(body=self.make_response_body())
        UsersFetcher(screen_name="jill").fetch([460060168, 26727655, 6795192])
        self.assertEqual("Aiannucci", User.objects.get(twitter_id=26727655).screen_name)
...
class TweetsFetcherTestCase(TwitterFetcherTestCase):
    ...
    def test_makes_one_api_call(self, download):
        ...
        TweetsFetcher(screen_name="jill").fetch([300, 200, 100])
        self.assertEqual(1, len(responses.calls))
    ...
    def test_requests_all_tweets(self):
        ...
        TweetsFetcher(screen_name="jill").fetch()
    ...
    def test_updates_tweets(self):
        TweetFactory.create(twitter_id=200, text="Will change")
        self.add_response(body=self.make_response_body(), method="POST")
        TweetsFetcher(screen_name="jill").fetch([300, 200, 100])
        self.assertEqual(
...
class VerifyFetcherTestCase(TwitterFetcherTestCase):
    ...
    def test_api_request_for_one_account(self, fetch_avatar):
        self.add_response(body=self.make_response_body())
        VerifyFetcher(screen_name="jill").fetch()
        self.assertEqual(1, len(responses.calls))
    ...
    def test_saves_users(self):
        ...
        user = UserFactory(
            twitter_id=12552, screen_name="philgyford", name="This should change"
        )
        AccountWithCredentialsFactory(id=4, user=user)

        self.add_response(body=self.make_response_body())
        VerifyFetcher(screen_name="philgyford").fetch()
...
class FilesFetcherTestCase(TestCase):
    def setUp(self):
        self.fetched_image = PhotoFactory(
            twitter_id=99999999, image_file="downloaded.jpg"
        )
    ...
    def test_calls_fetch_files_with_all(self, fetch):
        FilesFetcher().fetch(fetch_all=True)
    ...
    def test_downloads_image(self, download):
        ...
        FetchFiles()._fetch_and_save_file(media_obj=self.image, media_type="image")
    ...
    def test_downloads_video(self, download):
        ...
        download.return_value = False
        FetchFiles()._fetch_and_save_file(media_obj=self.animated_gif, media_type="mp4")
        download.assert_has_calls([call(self.animated_gif.mp4_url, ["video/mp4"])])
