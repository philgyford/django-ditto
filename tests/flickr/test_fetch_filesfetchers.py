...
class FilesFetcherTestCase(TestCase):
    def setUp(self):
        ...
        self.photo_3 = PhotoFactory(title="p3", original_file=None, user=UserFactory())

    def test_fails_with_no_account(self):
        with self.assertRaises(TypeError):
            OriginalFilesFetcher()

    def test_fails_with_no_flickr_user(self):
        fetcher = OriginalFilesFetcher(account=AccountFactory(user=None))
        results = fetcher.fetch()
        self.assertFalse(results["success"])
        self.assertIn("This account has no Flickr User", results["messages"][0])
    ...
    def test_calls_fetch_and_save_all(self, fetch_and_save_file):
        ...
        self.fetcher.fetch(fetch_all=True)
        calls = [
    ...
    def test_downloads_video(self, download):
        ...
        download.assert_has_calls(
            [call(self.video_2.video_original_url, ["video/mp4"])]
        )
