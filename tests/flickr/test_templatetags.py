...
class TemplatetagsRecentPhotosTestCase(TestCase):
    def setUp(self):
        ...
        self.public_pks = [p.pk for p in self.photos_1] + [p.pk for p in self.photos_2]
...
class TemplatetagsDayPhotosTestCase(TestCase):
    def setUp(self):
        ...
        self.photos_2[1].taken_time = taken_time + timedelta(hours=1)
        self.photos_2[1].post_time = post_time + timedelta(hours=1)
        self.photos_2[1].save()

        self.private_photo = PhotoFactory(
            user=user_1, is_private=True, post_time=post_time
        )

    def test_day_photos(self):
        "Returns only public Photos from the date"
        photos = ditto_flickr.day_photos(date(2015, 3, 18))
        self.assertEqual(2, len(photos))
...
class PhotoLicenseTestCase(TestCase):
    def test_license_0(self):
        self.assertEqual(ditto_flickr.photo_license("0"), "All Rights Reserved")

    def test_license_1(self):
        self.assertEqual(
            ditto_flickr.photo_license("1"),
            (
...
class AnnualPhotoCountsPostTimeTestCase(TestCase):
    def setUp(self):
        ...
        PhotoFactory.create_batch(
            3, post_time=datetime_from_str("2015-01-01 12:00:00"), user=self.user_1
        )
        PhotoFactory.create_batch(
            2, post_time=datetime_from_str("2016-01-01 12:00:00"), user=self.user_1
        )
        # And one for user_2 posted in 2015:
        PhotoFactory(
            user=self.user_2, post_time=datetime_from_str("2015-01-01 12:00:00")
        )
    ...
    def test_empty_years(self):
        ...
        PhotoFactory(
            post_time=datetime_from_str("2018-01-01 12:00:00"), user=self.user_1
        )
        photos = ditto_flickr.annual_photo_counts()
...
class AnnualPhotoCountsTakenTimeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        PhotoFactory.create_batch(
            3, taken_time=datetime_from_str("2015-01-01 12:00:00"), user=self.user_1
        )
        PhotoFactory.create_batch(
            2, taken_time=datetime_from_str("2016-01-01 12:00:00"), user=self.user_1
        )
        # And one for user_2 taken in 2015:
        PhotoFactory(
            user=self.user_2, taken_time=datetime_from_str("2015-01-01 12:00:00")
        )
    ...
    def test_response_for_user(self):
        "Returns correct data for one user."
        photos = ditto_flickr.annual_photo_counts(
            nsid="1234567890@N01", count_by="taken_time"
        )
        self.assertEqual(len(photos), 2)
        self.assertEqual(photos[0]["year"], 2015)
        self.assertEqual(photos[0]["count"], 3)
    ...
    def test_empty_years(self):
        ...
        # Add a photo in 2018, leaving a gap for 2017:
        PhotoFactory(
            taken_time=datetime_from_str("2018-01-01 12:00:00"), user=self.user_1
        )
        photos = ditto_flickr.annual_photo_counts(count_by="taken_time")
        self.assertEqual(len(photos), 4)
        self.assertEqual(photos[2]["year"], 2017)
